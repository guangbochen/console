...
secretname=$(kubectl get serviceaccount cattle --namespace=cattle-system -o jsonpath='{.secrets[0].name}')
BRIDGE_K8S_AUTH_BEARER_TOKEN=$(kubectl get secret "$secretname" --namespace=cattle-system -o template --template='{{.data.token}}' | base64 --decode)
